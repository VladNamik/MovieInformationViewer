        ...
        if (searchPage != null) {
            ...
            BroadcastReceiver loadPageMessageReceiver = new BroadcastReceiver() {
                ...
                public void onReceive(Context context, Intent intent) {
                    Log.d(LOG_TAG, "start search page onReceive()");
                    ...
                    Log.d(LOG_TAG, "extraQuery = " + extraQuery + "; extraPageNumber = "
                    ...
                    if (extraQuery.equals(search) && extraPageNumber == pageNumber) {
                        ...
                        Log.d(LOG_TAG, "end search page onReceive()");
                        broadcastLock.lock();
            ...
            try {
                ...
                Log.d(LOG_TAG, "start waiting for broadcast end (search page)");
                broadcastEndCondition.await(10, TimeUnit.SECONDS);
                Log.d(LOG_TAG, "end waiting for broadcast end (search page)");
    ...
    public Movie loadFullMovieByImdbId(Context context, final String imdbId) throws IOException {
        ...
        if (movie != null && movie.isFull()) {
            ...
            BroadcastReceiver loadFullMovieInfoMessageReceiver = new BroadcastReceiver() {
                @Override
                public void onReceive(Context context, Intent intent) {
                    Log.d(LOG_TAG, "start movie full info onReceive()");
                    String extraMovieImdbId = intent.getStringExtra(APIService.EXTRA_MOVIE_IMDB_ID);
                    Log.d(LOG_TAG, "extraMovieImdbId = " + extraMovieImdbId + "; MovieImdbId = " + imdbId);

                    if (extraMovieImdbId.equals(imdbId)) {
                        status.append(intent.getStringExtra(APIService.EXTENDED_DATA_STATUS));
                        Log.d(LOG_TAG, "end movie full info onReceive()");
                        broadcastLock.lock();
                        broadcastEndCondition.signal();
                        broadcastLock.unlock();
                    }
            ...
            try {
                ...
                Log.d(LOG_TAG, "start waiting for broadcast end (movie full info)");
                broadcastEndCondition.await(10, TimeUnit.SECONDS);
                Log.d(LOG_TAG, "end waiting for broadcast end (movie full info)");
            } catch (InterruptedException e) {
                e.printStackTrace();
            } finally {
